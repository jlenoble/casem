...
var literalNames = [ null, "'('", "')'", "','", "'\\->'", "'\\cos '", "'\\Locate '", 
                     "'\\Pi'", "'\\sin '", "'\\tan '", null, null, null, 
                     "'-'", "'+'", "'/'", "'*'", null, "'0'" ];

var symbolicNames = [ null, null, null, null, "ARROW", "COS", "LOCATE", 
                      "PI", "SIN", "TAN", "ID", "NATNUM", "NEWLINE", "MINUS", 
                      "PLUS", "SLASH", "STAR", "STRING", "ZERO" ];

var ruleNames =  [ "prog", "statement", "expr", "evalExpr", "printExpr", 
                   "assignExpr", "stoExpr", "additiveOp", "multiplicativeOp", 
                   "variable", "number", "func", "endStatement" ];
...
CalcParser.RULE_func = 11;
CalcParser.RULE_endStatement = 12;
...
CalcParser.prototype.prog = function() {
    ...
    try {
        ...
        do {
            ...
            this._errHandler.sync(this);
            _la = this._input.LA(1);
        } while((((_la) & ~0x1f) == 0 && ((1 << _la) & ((1 << CalcParser.T__0) | (1 << CalcParser.COS) | (1 << CalcParser.LOCATE) | (1 << CalcParser.PI) | (1 << CalcParser.SIN) | (1 << CalcParser.TAN) | (1 << CalcParser.ID) | (1 << CalcParser.NATNUM) | (1 << CalcParser.MINUS) | (1 << CalcParser.STRING) | (1 << CalcParser.ZERO))) !== 0));
    } catch (re) {
...
CalcParser.prototype.func = function() {
    ...
    this.enterRule(localctx, 22, CalcParser.RULE_func);
    var _la = 0; // Token type
    try {
        this.enterOuterAlt(localctx, 1);
        this.state = 90;
        _la = this._input.LA(1);
        if(!((((_la) & ~0x1f) == 0 && ((1 << _la) & ((1 << CalcParser.COS) | (1 << CalcParser.SIN) | (1 << CalcParser.TAN))) !== 0))) {
        this._errHandler.recoverInline(this);
        }
        else {
        	this._errHandler.reportMatch(this);
...
CalcParser.prototype.endStatement = function() {
    ...
    this.enterRule(localctx, 24, CalcParser.RULE_endStatement);
    try {
        this.enterOuterAlt(localctx, 1);
        this.state = 92;
