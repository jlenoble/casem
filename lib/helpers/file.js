...
var File = function (_Block) {
  ...
  function File(ctx, visitor) {
    ...
    Object.defineProperty(_this, 'jumpingTo', {
      value: '',
      writable: true
    });
    return _this;
  ...
  _createClass(File, [{
    ...
    value: function reduce() {
      ...
      return _get(File.prototype.__proto__ || Object.getPrototypeOf(File.prototype), 'reduce', this).call(this).catch(function (err) {
        if (err.message === 'RETURN') {
          return;
        }

        if (('' + err.message).match(/^\\Lbl [A-Z0-9]$/)) {
          _this2.startJumping(err.message);
    ...
    value: function startJumping(label) {
      if (this.isJumping()) {
        throw new Error('Already jumping to ' + this.jumpingTo + '; Can\'t go to ' + label);
      }
    ...
    value: function stopJumping(label) {
      if (!this.isJumpingTo(label)) {
        throw new Error('Can\'t jump to ' + label + ' because jumping to ' + this.jumpingTo);
      }
      this.jumpingTo = '';
    }
  }]);
